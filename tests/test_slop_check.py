...
def test_flags_hedging():
    ...
    assert any(
        f.pattern == "hedging" for f in slop_check("It's worth noting this is arguably true.")
    )
...
def test_clean_technical_prose_not_flagged():
    ...
    clean = (
        "A neuron sums the previous layer's activations, each scaled by a weight, then adds "
