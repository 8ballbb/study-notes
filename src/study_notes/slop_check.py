...
_RULES: list[tuple[str, re.Pattern]] = [
    ...
    ("binary-contrast", re.compile(r"\bit'?s not (just )?[^.\n]{1,50}?,?\s*it'?s\b", re.I)),
    (
        "faux-insight",
        re.compile(
            r"\b(what (most people|nobody|everyone) (gets? wrong)|what nobody tells you|"
            r"the part (everyone|most people) miss(es)?)\b",
            re.I,
        ),
    ),
    (
        ...
        re.compile(
            ...
            r"plays? a (vital|crucial|pivotal) role)\b",
            re.I,
        ),
    ),
    (
        "weasel-attribution",
        re.compile(
            r"\b(experts agree|studies show|research shows|widely regarded as|"
            r"it is (widely )?believed)\b",
            re.I,
        ),
    ),
    (
    ...
    ("rhetorical-setup", re.compile(r"\b(what if i told you|think about it|plot twist)\b", re.I)),
    (
        "negative-listing",
        re.compile(r"\bnot (a |an )?[^.\n]{1,30}\.\s*not (a |an )?[^.\n]{1,30}\.", re.I),
    ),
    ("emoji-heading", re.compile(r"(?:^|\n)#{1,6} .*[\U0001F000-\U0001FAFF☀-➿]")),
    (
        "cliche-word",
        re.compile(
            r"\b(delve|delving|tapestry|leverage|leveraging|seamless(ly)?|elevate|elevates|"
    ...
    (
        "hedging",
        re.compile(
            r"\b(it'?s worth noting|it is worth noting|arguably|to some extent|"
            r"in many ways)\b",
            re.I,
        ),
    ),
